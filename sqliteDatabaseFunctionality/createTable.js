...
export const createPartyTable = () => {
	...
	return new Promise((resolve, reject) => {
		databaseObject.transaction(tx => {
			tx.executeSql(
				'CREATE TABLE IF NOT EXISTS party_table (id INTEGER PRIMARY KEY AUTOINCREMENT, first_name TEXT, last_name TEXT, mobile_number INTEGER, email TEXT, work_type TEXT, length INTEGER, width INTEGER, height INTEGER, total_area INTEGER, amount INTEGER, discount INTEGER)',
				[],
				(tx, results) => {
					resolve('Table created successfully');
				},
